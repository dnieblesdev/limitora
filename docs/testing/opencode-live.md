...
The process environment must contain only the supported
`LIMITORA_OPENCODE_API_KEY` input. Missing, empty, malformed, or wrong
confirmation input returns the constant `preflight` result. The child receives
only that API key and `PYTHONNOUSERSITE=1`; it never inherits
...
no valid quota window, `30` invalid UTF-8 JSON, and `31` non-object JSON root.
The `29` classification is the existing exact no-valid-window message; `30`
and `31` are parse-cause refinements.
...
are refined to `parse_failed_invalid_utf8_json` or
`parse_failed_non_object_json`; unknown parse messages remain `parse_failed`.
...
1. Before storing the secret or running the workflow, create and configure a
   protected GitHub Environment named `opencode-live` in repository or
   organization settings, including required reviewers.
2. Store `LIMITORA_OPENCODE_API_KEY` only after that protected Environment is
   configured.
...
contract before the API key is mapped. The verified wheelhouse is the only package
...
`parse_failed_invalid_utf8_json`, `parse_failed_non_object_json`, `rate_limited`,
`source_unavailable`, `transport`,
...
own classifications; the existing no-valid-window message and the two known
OpenCode parse causes use constant classifications; every non-success
