...
class OpenCodeLiveWorkflowTests(unittest.TestCase):
    ...
    def test_artifact_verification_and_offline_install_are_scoped(self):
        ...
        self.assertNotIn("digest-mismatch:", download)
        self.assertIn('artifact.get("digest") != os.environ["EXPECTED_DIGEST"]', verify)
        self.assertIn("manifest.sha256", manifest)
        self.assertIn("receipt.get(\"source_sha\") != expected_sha", manifest)
        self.assertIn('hashlib.sha256(path.read_bytes()).hexdigest() != digest', manifest)
        self.assertIn('receipt.get("manifest_sha256")', manifest)
        packaging_install = 'python -m pip install --disable-pip-version-check --no-index --find-links "$wheelhouse" "packaging==26.0"'
        self.assertIn(packaging_install, distribution)
        self.assertIn("scripts/verify_distributions.py", distribution)
        self.assertLess(distribution.index(packaging_install), distribution.index("python scripts/verify_distributions.py"))
